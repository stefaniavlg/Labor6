...
public class AddTeacherScene {
    /**
     * creates a Stage to add a teacher in
    ...
    public static Stage getScene(AdminController adminController){
        ...
        Label lb_title = new Label("Add new Teacher");
        ...
        btn_submit.setOnAction((ActionEvent e)->{
            ...
            if(adminController.existsTeacher(first,last)){
                lb_error.setText("Teacher already exists!");
            }
            else {
                adminController.addTeacher(first, last);
                stage.close();
            }
        });
