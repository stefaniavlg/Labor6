...
public class UpdateStudentScene {
    /**
     * creates a Stage to update a student in
    ...
    public static Stage getScene(AdminController adminController){
        ...
        Label lb_title = new Label("Update Student");
        ...
        RadioButton radioCourses = new RadioButton("Enrolled courses");
        radioCourses.setFont(new Font("Arial", 14));
        radioCourses.setUserData("enrolled");
        ...
        btn_submit.setOnAction((ActionEvent e)->{
            ...
            if(adminController.existsStudent(first,last)) {
                if (option.equals("first")) {
                    ...
                    if (adminController.existsStudent(change, last)) {
                        lb_error.setText("Student with new first name already exists");
                    } else {
                        adminController.updateStudent(first, last, change, 1);
                        stage.close();
                    }
                } else if (option.equals("last")) {
                    String change = tf_change.getText();
                    if (adminController.existsStudent(first, change)) {
                        lb_error.setText("Student with new last name already exists");
                    } else {
                        adminController.updateStudent(first, last, change, 2);
                        stage.close();
                    }
                }
                else if (option.equals("enrolled")) {
                    String change = tf_change.getText();
                    int updaterez = adminController.updateStudent(first, last, change, 3);
                    if (updaterez == 0) { lb_error.setText(""); stage.close();}
                    else { lb_error.setText("Course has already reached max enrollment, update impossible"); }
                }
            }
            else{
                lb_error.setText("Student doesn't exist");
            }
        });
